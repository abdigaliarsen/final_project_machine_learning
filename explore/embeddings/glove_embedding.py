...
glove_file = os.path.join(project_directory, "vectors", "glove.6B.300d.txt")

# Define the output submission path
submission_path = os.path.join(project_directory, "submissions", "glove_embedding_submission.csv")
...
embedding_dim = 300  # GloVe 300-dimensional vectors
embedding_index = {}
...
embedding_matrix = np.zeros((10000, embedding_dim))  # 10000 is the size of our vocabulary
...
# Model definition with pre-trained GloVe embeddings
model = Sequential([
    Embedding(input_dim=10000, output_dim=embedding_dim, input_length=max_length, weights=[embedding_matrix], trainable=False),  # Freeze embeddings
